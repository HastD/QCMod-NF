// This is a minimally modified version of qc_modular.m that extracts test data rather than printing output.
...
test_format := recformat<Q,level,p,N,prec,r,Delta,s,h1basis,g,rho,W0,cpm_prec,cpm,sympl_coeffs,basis0,basis1,basis2,tadicprec,
                    Qpoints,Qppoints,bad_Qppoints,good_Qpoints,good_affine_rat_pts_xy,bad_affine_rat_pts_xy,bQ,bpt,ks,teichpoints,
                    q,corresp,Tq,corr_loss,eqsplit,
                    hodge_prec,eta,betafil,gammafil,hodge_loss,Nhodge,
                    b0pt,NG,G_list,PhiAZb_to_b0,PhiAZb,PhiAZb_to_z,gammafil_listb_to_z,
                    ptrans_P1, ptrans_P2, ptrans_out, ptrans_N, changebasis,Phii,E1_E2,gammafilP,heights,
...
function QCModAffine_test_data(Q, p :
  ...
  test`r := r; test`Delta := Delta; test`s := s;
  ...
  if cpm ne standard_sympl_mat then 
    ...
    test`sympl_coeffs := coefficients;
  ...
  test`basis0 := basis0; test`basis1 := basis1; test`basis2 := basis2;
  ...
  test`good_Qpoints := good_Qpoints;
  ...
  test`good_affine_rat_pts_xy := good_affine_rat_pts_xy;
  ...
  test`bad_affine_rat_pts_xy := bad_affine_rat_pts_xy;
  ...
  test`corresp := correspondences; test`Tq := Tq; test`corr_loss := corr_loss;
  ...
  test`hodge_prec := [* *]; test`eta := [* *]; test`betafil := [* *]; test`gammafil := [* *]; test`hodge_loss := [* *]; test`Nhodge := [* *];
  test`b0pt := [* *]; test`NG := [* *]; test`G_list := [* *]; test`PhiAZb_to_b0 := [* *]; test`PhiAZb := [* *];
  test`PhiAZb_to_z := [* *]; test`gammafil_listb_to_z := [* *]; test`ptrans_P1:=[**]; test`ptrans_P2:=[**]; test`ptrans_out:=[**];
  test`ptrans_N := [**]; test`changebasis := [**]; test`Phii:=[**]; test`E1_E2 := [**]; test`gammafilP := [**];
  test`heights := [**];
  for l := 1 to number_of_correspondences do
    ...
    Append(~test`hodge_prec, hodge_prec);
    Append(~test`eta, eta);
    Append(~test`betafil, betafil);
    Append(~test`gammafil, gammafil);
    Append(~test`hodge_loss, hodge_loss);

    Nhodge := Ncorr + Min(0, hodge_loss);
    Append(~test`Nhodge, Nhodge);
    ...
    Append(~test`PhiAZb_to_b0, PhiAZb_to_b0);
    ...
    Append(~test`gammafil_listb_to_z, [Eltseq(gf) : gf in gammafil_listb_to_z]);
    ...
    if #height_coeffs eq 0 or not use_log_basis then // Compute heights of auxiliary points.
      ...
      if IsZero(E1P) then  // Find a point with non-zero E1 to write down a basis of the Lie algebra. 
        ...
        for i := 1 to #good_affine_rat_pts_xy_no_bpt do
          ...
          Append(~test`ptrans_P1, Qppoints[ks[i]]);
          Append(~test`ptrans_P2, Qpti);
          Append(~test`ptrans_out, pti);
          Append(~test`ptrans_N, Npti);
      ...
      Append(~test`changebasis, changebasis);
    ...
    if #height_coeffs eq 0 then // Compute heights of auxiliary points.
      if #heights lt g then  // add E1_E2(P) to known subspace until dimension is g.
        ...
        repeat 
          ...
          Append(~test`Phii, Phii); Append(~test`E1_E2, E1_E2_P);
          ...
          if Dimension(new_E1_E2_subspace) gt Dimension(E1_E2_subspace) then
            ...
            Append(~test`gammafilP, gammafilP); Append(~test`heights, height_P);
  ...
  test`height_coeffs := height_coeffs; test`Nhtcoeffs := Nhtcoeffs;
  ...
  test`zeroes_lists := zeroes_lists;
  test`sols_lists := sols_lists;
  if pl gt 0 then
...
//    P := [lindepQp(sols[i,1]), lindepQp(sols[i,2])];
  ...
  test`good_affine_rat_pts_xy_final := good_affine_rat_pts_xy;
  test`fake_rat_pts := fake_rat_pts;
  if #fake_rat_pts gt 0 then
    return good_affine_rat_pts_xy, false, bad_affine_rat_pts_xy, data, fake_rat_pts, bad_Qppoints, test;
  else
    return good_affine_rat_pts_xy, true, bad_affine_rat_pts_xy, data, fake_rat_pts, bad_Qppoints, test;
  end if;

end function;


function QCModQuartic_test_data(Q, S :
  ...
  assert LeadingCoefficient(Q) eq 1;
  // TODO: 
  ...
  while p lt bound do
    ...
    if (not IsDivisibleBy(Level(S), p)) and HeckeOperatorGenerates(S, p) then
      ...
      return true, Cpts, p, Q_inf, test_data1, test_data2;  
...
for i := 1 to #x0plus_quartics_list do
  ...
  time success, Cpts, p, Q_inf, test_data, test_data_inf := QCModQuartic_test_data(Q, S : printlevel := 2, N := 20);
